/**
 * Gesture state machine. Pure on purpose: no React, no three, no telemetry,
 * so every control defect is testable without a browser.
 */
...
/** `dragTool` tells brush and eraser apart: only they paint while dragging. */
...
/**
 * OrbitControls listens on the canvas, outside R3F: while a gesture may still
 * become a stroke the camera has to stay frozen or it drifts.
 */
const orbitIn = (phase: Phase) => phase !== 'pending' && phase !== 'painting'
...
export function step(
  ...
  if (
    ...
    // The single way out. There used to be four, and three left orbit dead.
    return {
      state: idle(),
      out: {
        phase: 'idle',
  ...
  if (input.kind === 'down') {
    const toCamera = ctx.mode === 'navigate' || input.button !== 0 || input.cell === null
    if (toCamera) {
      return {
        state: {
          ...idle(),
  ...
  if (input.kind === 'move' && state.phase === 'pending') {
    ...
    if (!ctx.dragTool) {
      // Line, rectangle, fill, selection and eyedropper never paint by dragging.
      return {
  ...
  return { state, out: { phase: state.phase, orbitEnabled: orbitIn(state.phase) } }
}
