...
import {
  initialState, step,
  type Cell, type Ctx, type Input, type Mods,
} from '../src/scene/gesture'
...
const downAt = (x: number, y: number, cell: Cell | null, pointerType = 'mouse'): Input => ({
  kind: 'down', pointerId: 1, pointerType, button: 0, x, y, cell, mods: NO_MODS,
})

/* ── gesture: click vs. drag ─────────────────────────────────────────────── */

test('un down seguido de up sin movimiento es un click y commitea una celda', () => {
  const down = step(initialState, downAt(100, 100, { x: 5, y: 0, z: 5 }), BUILD)

  expect(down.state.phase).toBe('pending')
  expect(down.out.capture).toBe(1)
  expect(down.out.commit).toBeUndefined()
  // La órbita queda apagada mientras el gesto puede volverse trazo: los
  // listeners de OrbitControls no pasan por R3F.
  expect(down.out.orbitEnabled).toBe(false)
...
/* ── gesture: los cuatro caminos de cancelación ──────────────────────────── */
...
/* ── picking: una sola fuente de verdad ──────────────────────────────────── */
...
test('shift convierte colocar en borrar sobre el bloque apuntado', () => {
  const r = resolveCell(
    hitOn('block', [7.5, 2, 7.5], [0, 1, 0]), { ...NO_MODS, shift: true }, 'brush', DIMS,
  )
...
test('alt es cuentagotas sobre el bloque apuntado', () => {
  const r = resolveCell(
    hitOn('block', [7.5, 2, 7.5], [0, 1, 0]), { ...NO_MODS, alt: true }, 'brush', DIMS,
  )
...
test('en el plano de capa se pinta y se borra la misma celda', () => {
  const place = resolveCell(hitOn('slice', [4.3, 3.5, 8.7], [0, 1, 0]), NO_MODS, 'brush', DIMS)
  expect(place.chosen).toEqual({ x: 4, y: 3, z: 8 })
  expect(place.face).toBeNull()

  const erase = resolveCell(hitOn('slice', [4.3, 3.5, 8.7], [0, 1, 0]), NO_MODS, 'eraser', DIMS)
  expect(erase.chosen).toEqual({ x: 4, y: 3, z: 8 })
  expect(erase.action).toBe('erase')
})

test('la cara lateral resuelve sin error de redondeo', () => {
  const r = resolveCell(hitOn('block', [6, 0.5, 3.5], [-1, 0, 0]), NO_MODS, 'brush', DIMS)
  expect(r.target).toEqual({ x: 6, y: 0, z: 3 })
  expect(r.placement).toEqual({ x: 5, y: 0, z: 3 })
})

/* ── modeKeys: toque vs. mantener ────────────────────────────────────────── */

test('un toque corto de Space alterna de build a navigate', () => {
  const d = spaceDown('build', 1000)
  expect(d.mode).toBe('navigate')
  expect(spaceUp(d.state, 1100, false).mode).toBe('navigate')
})

test('un toque corto de Space alterna de navigate a build', () => {
  const d = spaceDown('navigate', 1000)
  expect(d.mode).toBe('navigate')
  expect(spaceUp(d.state, 1100, false).mode).toBe('build')
})

test('mantener Space y soltar restaura el modo previo', () => {
  const d = spaceDown('build', 1000)
  expect(spaceUp(d.state, 1600, false).mode).toBe('build')
})

test('mover la cámara mientras se mantiene lo hace transitorio aunque sea rápido', () => {
  const d = spaceDown('build', 1000)
  expect(spaceUp(d.state, 1100, true).mode).toBe('build')
})
